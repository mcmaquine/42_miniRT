...
/*   Updated: 2026/08/08 11:58:11 by mmaquine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_light(char **params, t_light *light, int line)
{
	...
	{
		print_error(OBJ_LIGHT, ERR_OUT_RANGE_BRIGHT, line);
		return (1);
	}
	if (fill_coordinate(params[1], &(light->coord), OBJ_LIGHT, line)
		|| fill_color(params[3], &(light->color), OBJ_LIGHT, line))
		return (1);
	return (0);
}
