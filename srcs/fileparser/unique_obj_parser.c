...
static int	check_unique(void *object, char **params, t_objs_type obj,
		size_t size)
{
	if (object != NULL)
	{
		print_error(obj, ERR_NO_UNIQUE, 0);
		return (1);
	}
	if (ft_sizeof_split(params) != size)
	{
...
static int	parse_ambient_rate(char *param, double *rate)
{
...
int	amb_light_parser(char **params, t_scene *scene_obj)
{
	...
	if (check_unique(scene_obj->amb, params, OBJ_AMBIENT, 3))
		return (1);
